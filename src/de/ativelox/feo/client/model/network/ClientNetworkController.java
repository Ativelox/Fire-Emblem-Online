...
public class ClientNetworkController extends ANetworkController<EC2S, ES2C> {
	...
	switch (protocol) {
	case ATTACK:
	    mPc.onAttackReceived(decodeUnitToId(additional[0]), decodeUnitToId(additional[1]),
		    decodePath(additional[2]));
	    break;
	case END_TURN:
	    mPc.onTurnEndReceived();
	    break;
	case WAIT:
	    mPc.onWaitReceived(decodeUnitToId(additional[0]), decodePath(additional[1]));
	    break;
	case WELCOME:
	    mPc.onServerWelcome(Integer.parseInt(additional[0]), Long.parseLong(additional[1]));
	    break;
	default:
    ...
    private static int decodeUnitToId(String encodedUnit) {
	return Integer.parseInt(encodedUnit);
